...
class NurikabeSolver(GameSolver):
    ...
    def _add_constraints(self):
        ...
        self._add_river_around_islands_constraint()
    ...
    def _add_initial_constraints(self):
        for i, position in enumerate(self.islands_size_position):
            self._model.Add(self._grid_z3[position] == i + 1)
        ...
        for position in self.islands_size_position:
            if self._grid[position] == 1: # Island of size 1
                for neighbor in self._grid.neighbors_positions(position):
                    self._model.Add(self._grid_z3[neighbor] == 0)
    ...
    def add_connected_cells_in_region_constraints(self, step_vars, region_id, region_size):
        ...
        for position, value in self._grid_z3:
            is_cell_region = self._model.NewBoolVar(f'is_cell_region_{region_id}_{position}')
            self._model.Add(self._grid_z3[position] == region_id).OnlyEnforceIf(is_cell_region)
            self._model.Add(self._grid_z3[position] != region_id).OnlyEnforceIf(is_cell_region.Not())
        ...
        root_bool_vars = []
        for position, value in self._grid_z3:
            is_cell_region_for_root = self._model.NewBoolVar(f'is_cell_region_for_root_{region_id}_{position}')
            self._model.Add(self._grid_z3[position] == region_id).OnlyEnforceIf(is_cell_region_for_root)
            self._model.Add(self._grid_z3[position] != region_id).OnlyEnforceIf(is_cell_region_for_root.Not())

            is_step_one = self._model.NewBoolVar(f'is_step_one_{region_id}_{position}')
            self._model.Add(step_vars[position] == 1).OnlyEnforceIf(is_step_one)
            self._model.Add(step_vars[position] != 1).OnlyEnforceIf(is_step_one.Not())

            is_root = self._model.NewBoolVar(f'is_root_{region_id}_{position}')
    ...
    def _add_island_regions_constraints(self):
        ...
        steps_vars_per_island = [
            Grid([[self._model.NewIntVar(0, max_island_size, f"step_island_{island_idx}_{r}_{c}") for c in range(self.columns_number)] for r in range(self.rows_number)])
    ...
    def _add_river_between_2_island_area_constraint(self):
        for r in range(self.rows_number - 2):
            for c in range(self.columns_number - 2):
                ...
                if pos_down_2 in self._grid and self._grid[pos_down_2] > 0:
                    middle_position = position.after(Direction.down(), 1)
                    self._model.Add(self._grid_z3[middle_position] == 0)

                pos_right_2 = position.after(Direction.right(), 2)
                if pos_right_2 in self._grid and self._grid[pos_right_2] > 0:
                    middle_position = position.after(Direction.right(), 1)
                    self._model.Add(self._grid_z3[middle_position] == 0)

    def _add_river_region_constraint(self):
        region_id_river = 0

        river_step_vars = [[self._model.NewIntVar(0, self._river_size, f"step_river_{r}_{c}") for c in range(self.columns_number)] for r in range(self.rows_number)]
    ...
    def _add_river_around_islands_constraint(self):
        for i, position in enumerate(self.islands_size_position):
            island_id = i + 1
